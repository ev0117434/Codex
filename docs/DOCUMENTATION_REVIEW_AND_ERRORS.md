...
- Выполнена повторная проверка ссылок между markdown-документами — битых ссылок не обнаружено.
- Дополнительно уточнены обозначения единиц времени в stale-правиле (`*_NS`) для согласованности контрактов.
- Исправлена формулировка в `ASSUMPTIONS_AND_LIMITS.md` для однозначного описания поведения snapshot при потере рынков.
